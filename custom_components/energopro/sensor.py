...
"""Sensors for ENERGOPRO Bulgaria."""
...
async def async_setup_entry(
    ...
    """Set up ENERGOPRO sensors."""
    coordinator: ENERGOPROCoordinator = entry.runtime_data

    async_add_entities(
        [
            ENERGOPRODayPriceSensor(coordinator),
            ENERGOPRODayPriceVatSensor(coordinator),
            ENERGOPRONightPriceSensor(coordinator),
            ENERGOPRONightPriceVatSensor(coordinator),
            ENERGOPROCurrentPriceSensor(coordinator),
            ENERGOPROCurrentPriceVatSensor(coordinator),
            ENERGOPROCurrentTariffSensor(coordinator),
        ]
    )


class ENERGOPROBaseSensor(
    CoordinatorEntity[ENERGOPROCoordinator],
    SensorEntity,
):
    """Base class for ENERGOPRO sensors."""
    ...
    def __init__(
        self,
        coordinator: ENERGOPROCoordinator,
    ) -> None:
...
def get_current_tariff(now: datetime) -> str:
    """Return the current ENERGOPRO tariff."""
    month = now.month
    hour = now.hour
